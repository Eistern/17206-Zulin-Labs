...
static option::ArgStatus PlayerType(const option::Option &option, bool msg) {
     if (option.arg != nullptr && (strcmp(option.arg, "=random") == 0 || strcmp(option.arg, "=console") == 0 || strcmp(option.arg, "=optimal") == 0))
...
enum optionIndex {ROUNDS, PLAYER, HELP};
enum optionType {FIRST, SECOND};

const option::Descriptor usage[] = {
        {ROUNDS, 0, "c", "count", Numeric, "-c or --count defines number of rounds Default: 1"},
        {PLAYER, FIRST, "f", "first", PlayerType, "-f or --first defines type of first player Default: random"},
        {PLAYER, SECOND, "s", "second", PlayerType, "-s or --second defines type of second player Default: random"},
        {HELP, 0, "h", "help", option::Arg::None, "-h or --help prints out usage of options"}
};
...
int main(int argc, char* argv[]) {
    ...
    for (option::Option* opt = options[PLAYER]; opt; opt = opt->next()) {
        switch (opt->type()) {
            case FIRST : if (_player1 == nullptr)
                            _player1 = factoryMethod(opt->arg);
                break;
            case SECOND: if (_player2 == nullptr)
